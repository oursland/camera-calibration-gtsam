...
class IntrinsicCalibration : public rclcpp::Node
{
public:
  explicit IntrinsicCalibration(rclcpp::NodeOptions const& options)
    : rclcpp::Node{ "intrinsic_calibration", options }
    , tag_count_(0)
    , frame_count_(0)
    ...
    , pose_count_(0)
  {
    ...
    image_mono_sub_ = image_transport::create_subscription(
      this, "image_mono",
      std::bind(
        &IntrinsicCalibration::image_cb, this, std::placeholders::_1),
      "raw");
    image_apriltag_pub_ = image_transport::create_publisher(this, "image_apriltags");

    calibrate_srv_ = create_service<mocap_msgs::srv::Calibrate>("calibrate", std::bind(&mocap::IntrinsicCalibration::calibrate, this, std::placeholders::_1, std::placeholders::_2));
  }

  void image_cb(const sensor_msgs::msg::Image::ConstSharedPtr & image_msg) {
    ...
    if (image_apriltag_pub_.getNumSubscribers()) {
      cv::Mat color;
      ...
      image_apriltag_pub_.publish(cv_img.toImageMsg());
    }
...
private:
  ...
  image_transport::Subscriber image_mono_sub_;
  image_transport::Publisher image_apriltag_pub_;
...
int main(int argc, char** argv)
{
  ...
  auto node = std::make_shared<mocap::IntrinsicCalibration>(rclcpp::NodeOptions{});
...
RCLCPP_COMPONENTS_REGISTER_NODE(mocap::IntrinsicCalibration)
